(async () => {
  ...
  const href = await fetch(`http://natannir.wixsite.com/editor-dev-tools/_functions/github?location=${window.location.href}`).then(x => x.json());

  if (href) {
    const title = document.querySelector('.tabs') || document.getElementById('restPageTitle');
